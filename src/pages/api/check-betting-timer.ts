...
function startBettingTimer(
	...
	const elapsedTime =
		(15 - initialTimer) * 1000 + processingTime + additionalBuffer;
	...
	logInterval = setInterval(() => {
		...
		if (currentState) {
			...
			console.log(
				`Cycle ${cycleNumber} - Position: ${Math.floor(
					cyclePosition / 1000
				)}s - startBetting: ${currentState.startBetting}, timer: ${
					currentState.timer
				}`
			);
		} else {
...
export function resetGlobalTimer(): void {
	...
	if (logInterval) {
		...
		logInterval = null;
		console.log(
			"Global timer reset due to 3 consecutive non-100 balance hands"
		);
	}
}
...
export function trackBalanceForTimerReset(balance: number): void {
	if (balance !== 100) {
		...
		console.log(
			`Non-100 balance detected: ${balance}, consecutive count: ${consecutiveNon100Count}`
		);

		if (consecutiveNon100Count >= 3) {
			console.log(
				"3 consecutive non-100 balance hands detected - resetting timer"
			);
			resetGlobalTimer();
		}
	} else {
...
export default async function handler(
	...
	try {
		...
		if (existingTimer) {
			console.log(
				`Global timer active - startBetting: ${existingTimer.startBetting}, timer: ${existingTimer.timer}, user: ${id}`
			);
...
Return only the JSON object with no additional text.`,
		...
		if (experimental_output.startBetting && experimental_output.timer > 0) {
			const processingTime = Date.now() - requestStartTime;
			console.log(
				`Starting global timer with AI detected timer: ${experimental_output.timer}, processing time: ${processingTime}ms`
			);
